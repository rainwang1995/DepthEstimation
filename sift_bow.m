...
%                                                           Setup data
% --------------------------------------------------------------------
classes = {'background','circle','pentagon','rectangle','star','triangle'};

images = {} ;
imageClass = {} ;
for ci = 1:length(classes)
  if(ci==1)
      ims = dir(fullfile(conf.shapedir, classes{ci}, '*.jpg'))' ;
      ims = vl_colsubset(ims, conf.numTrain + conf.numTest) ;
      ims = cellfun(@(x)fullfile(classes{ci},x),{ims.name},'UniformOutput',false) ;
      images = {images{:}, ims{:}} ;
      imageClass{end+1} = ci * ones(1,length(ims)) ;
  else
      ims = dir(fullfile(conf.shapedir, classes{ci}, '*.png'))' ;
      ims = vl_colsubset(ims, conf.numTrain + conf.numTest) ;
      ims = cellfun(@(x)fullfile(classes{ci},x),{ims.name},'UniformOutput',false) ;
      images = {images{:}, ims{:}} ;
      imageClass{end+1} = ci * ones(1,length(ims)) ;
...
if ~exist(conf.vocabPath) || conf.clobber
  ...
  selTrainFeats = vl_colsubset(selTrain, 45) ;
  descrs = {} ;
  %for ii = 1:length(selTrainFeats)
  parfor ii = 1:length(selTrainFeats)
    im = imread(fullfile(conf.shapedir, images{selTrainFeats(ii)})) ;
...
if ~exist(conf.histPath) || conf.clobber
  ...
  % for ii = 1:length(images)
    ...
    im = imread(fullfile(conf.shapedir, images{ii})) ;
